...
namespace UnityEngine.Rendering.SelfUniversal.Internal
{
    ...
    public class DepthOnlyPass : ScriptableRenderPass
    {
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            {
                this.OverrideCameraAttachment(renderingData.cameraData.renderer.depthRT, descriptor.depthStencilFormat, desc.width, desc.height, 1, true);
            }
            // When not using depth priming the camera target should be set to our non MSAA depth target.
            else
            {
                this.OverrideCameraAttachment(new RenderTargetIdentifier(depthAttachmentHandle.Identifier(), 0, CubemapFace.Unknown, -1), descriptor.depthStencilFormat, desc.width, desc.height, 1, true);
            }
